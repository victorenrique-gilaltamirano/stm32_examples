  ...
  *  PROGRAMA DE EJEMPLO DE GENERAR UNA SEÑAL PWM
	*
  *  Se enciende y apaga un LED conectado a un canal de un
  *  pin de salida Analogica, controlandose mediente una señal
  *  PWM el brillo del LED.
  *
  *  Conexiones:
  *
...
int main(void)
 {	
  ...
  while(1)
    ...
    // Espera a que se compare el valor resultante en el registro 1 de comparación del Timer_1
    // con el valor que tiene la variable 'LED_valor'
...
{  
  ...
  TIM_PWM_InitStructure.TIM_Pulse = Pulse; 
  ...
  // Canal 1 del Timer 1
	TIM_OC1Init(TIM1, &TIM_PWM_InitStructure);  
